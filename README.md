...
4. Install the app **only** on `ticoncreserv/app`. Do not grant `Administration`. After install or sign-in GitHub redirects to the **Callback URL**. Locally the authorize flow uses `http://localhost/api/auth/github/callback` and the loopback proxy forwards it to the listen port. Token exchange sends that same `redirect_uri` (stored in OAuth `state`) — not the `:43123` host after the proxy hop. In production set `ATELIER_PUBLIC_URL=https://your-domain` and register `{ATELIER_PUBLIC_URL}/api/auth/github/callback`.
