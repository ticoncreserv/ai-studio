...
describe("provider models", () => {
  it("passes a model as a flag only where the CLI documents one", () => {
    ...
    expect(modelArgs("codex", "gpt-5.4")).toEqual([]);
  });
  ...
  it("passes a model as an env var for the agents that read one", () => {
    ...
    expect(applyModelEnv("codex", { PATH: "/bin" }, "gpt-5.4")).toEqual({ PATH: "/bin" });
  ...
  it("merges catalog, discovered, and configured models without duplicates", () => {
    const merged = mergeProviderModels(
      [{ id: "auto", label: "Auto" }],
      [{ id: "auto", label: "Auto (discovered)", description: "fast" }],
      [{ id: "custom-1", label: "" }],
    );
    expect(merged.filter((row) => row.id === "auto")).toHaveLength(1);
    expect(merged.find((row) => row.id === "auto")?.description).toBe("fast");
    expect(merged.find((row) => row.id === "custom-1")?.label).toBe("custom-1");
    expect(providerModelCatalog("cursor").some((row) => row.id === "composer-2.5")).toBe(true);
  });
});
